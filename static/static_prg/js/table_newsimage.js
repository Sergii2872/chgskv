// Скрипт отрисовки таблицы плагина datatables.net для таблицы БД наименования валют NewsImage, подключен в blok2/newsimage.html
...
// методом ajax обмениваемся данными с функцией list класса NewsImage_ViewSet в blok2/views.py
...
let table = $('#datatables').DataTable({
    ...
    buttons: [
        ...
        {   extend: 'csv', exportOptions: {
                columns: [1, 2, 3, 4, 5, 6, 7], // выбор столбцов для csv файла
                }
        },
        {   extend: 'excel', exportOptions: {
                columns: [1, 2, 3, 4, 5, 6, 7], // выбор столбцов для excel файла
                }
        },
        {   extend: 'pdf', exportOptions: {
                columns: [1, 2, 3, 4, 5, 6, 7], // выбор столбцов для pdf файла
                }
        },
        {
            extend: 'print',
            ...
            title: 'Справочник фото новостей', // заголовок печати, если не задаем то печатает title html страницы
            autoPrint: true, // false - отключение всплывающего окна печати
            exportOptions: {
                //columns: ':visible',
                columns: [1, 2, 3, 4, 5, 6, 7], // выбор столбцов для печати
            },
    ...
    "columns": [
        ...
        {"data": "id"},
        {"data": "to_news.header", "defaultContent": "не задан"},
        //{"data": "image"}, //https://jsfiddle.net/glenderson/yg1w6gen/1/ вариант отображения изображения
        {
        "data": "image",
    ...
    'columnDefs': [{
        ...
        {'targets': [8], // поля отключена сортировка [P1,...,Pn]
         'searchable': false,
         'orderable': false
        }],
...
$('#datatables tbody').on('click', 'button', function () {
    ...
    var imagenUrl = data.image;
    console.log(imagenUrl);
    let class_name = $(this).attr('class');
    if (class_name == 'btn btn-info') {
        // EDIT button
        $('#to_news_id').val(data['to_news']['id']);
        //$('#image').val();
...
// функция кодировки файла изображения в base64, вызывается из input newsimage.html
...
$('#token').on('submit', function (e) {
        ...
        //var file = $('#image')[0].files[0]; получаем файл из input по id = image
        // получаем значение из скрытого input https://translated.turbopages.org/proxy_u/en-ru.ru.283a2e29-623c6d7c-06d1102a-74722d776562/https/stackoverflow.com/questions/68067599/convert-image-to-base64-and-upload-via-ajax
        var dataBase64 = encodeURIComponent($('#b64').val());
        console.log('RESULT:', dataBase64);
        data = $this.serialize() + '&image=' + dataBase64;
        console.log('Data:', data);
        let type = $('#type').val();
        let method = '';
        let url = '/api/NewsImage/';
