// не используется т.к. поменялась логика, отражена в table_name_currency_trading.js

// Скрипт отрисовки таблицы плагина datatables.net для таблицы БД наименования валют Name_Currency_Trading, подключен в blok2/name_currency_trading.html
...
// методом ajax обмениваемся данными с функцией list класса NameCurrencyTrading_ViewSet в blok2/views.py
...
let table = $('#datatables').DataTable({
    ...
    "columns": [
        ...
        {"data": "name_currency_sale_id"},
        {"data": "name_currency_sale_currency"},
        {"data": "id_pair_market"},
        {"data": "is_active"},
        {"data": "created"},
        {"data": "updated"},
    ...
    'columnDefs': [{
      ...
      'order': [[1, 'asc']] //столбец к которому применяется сортировка, определен в ORDER_COLUMN_CHOICES_NameCurrencyTrading blok2/models.py
...
$('#datatables tbody').on('click', 'button', function () {
    ...
    if (class_name == 'btn btn-info') {
        ...
        //$('#name_currency_sale_id').val(data['name_currency_sale_id']);
        //$('#name_currency_sale_currency').val(data['name_currency_sale_currency']);
...
// Подставляем в скрытое поле input(наименование валюты) по id=name_currency_sale_currency значение select id=name_currency_sale_id
$('#name_currency_sale_id').on('change', function(){
    val = $('#name_currency_sale_id option:selected').html();
    $('#name_currency_sale_currency').val(val);
});
...
$('#token').on('submit', function (e) {
    ...
    let type = $('#type').val();
    let method = '';
    let url = '/api/Name_Currency_Trading/';
...
$('#confirm').on('click', '#delete', function (e) {
    var csrf_token = $('#token [name="csrfmiddlewaretoken"]').val();
    $.ajax({
        url: '/api/Name_Currency_Trading/' + id + '/',
        headers:{"X-CSRFToken": csrf_token},
        method: 'DELETE'
    }).success(function (data, textStatus, jqXHR) {
        location.reload();
...
// задаем начальные значения модального окна(по id myModal) новой записи или редактирования в blok2/name_currency_trading.html
$('#new').on('click', function (e) {
    //$('#name_currency_id').val('1');
    //$('#name_currency_sale_id').val('');
...
// функция валидации выбора select вызываем из prices_currency.html
var valid = 0;
function ValidCurrency() {

    var location = document.getElementById('name_currency_sale_id');
    var invalid = location.value == "Сделайте выбор";

    if (invalid) {
        $.alert('Выберите продаваемую валюту!');
...
function ValidCurrency2() {
    ...
    if (invalid) {
        $.alert('Выберите покупаемую валюту!');
        location.className = 'error';
        valid = 1;
  ...
  $.confirm({
    ...
    buttons: {
        ...
        action: function () {
          ...
          //$('#chekbox').text($(form).serialize()); // текст для name_currency_trading.html
          ...
          var url = [];
          $(id).each(function(i){
                url.push('/api/Name_Currency_Trading/' + id[i] + '/');
                });
